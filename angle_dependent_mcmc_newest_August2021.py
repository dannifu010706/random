...
def FOM(x, color): 
    if color == 'blue':
        ...
        for i in range(len(lam_step)):
            for j in range(len(theta_step)):
                ...
                P_target.append(Gaussian((lam_step[i-1]+lam_step[i])/2,6161.31,lam_naught))
    ...
    if color == 'green':
        ...
        for i in range(len(lam_step)):
            for j in range(len(theta_step)):
                P_scat.append(subroutine_scat(x,((theta_step[j-1]+theta_step[j])/2),((lam_step[i-1]+lam_step[i])/2))) 
                P_target.append(Gaussian((lam_step[i-1]+lam_step[i])/2,1312.47,lam_naught))
        P_naught = max(P_scat) * ((np.pi) ** (1/2))
        P_scat = np.asarray(P_scat)
        P_target = np.asarray(P_target)
        P_scat = P_scat / P_naught
    ...
    if color == 'red':
        ...
        for i in range(len(lam_step)):
            for j in range(len(theta_step)):
                P_scat.append(subroutine_scat(x,((theta_step[j-1]+theta_step[j])/2),((lam_step[i-1]+lam_step[i])/2))) 
                P_target.append(Gaussian((lam_step[i-1]+lam_step[i])/2,985.809,lam_naught))
        P_naught = max(P_scat) * ((np.pi) ** (1/2))
        P_scat = np.asarray(P_scat)
        P_target = np.asarray(P_target)
        P_scat = P_scat / P_naught
