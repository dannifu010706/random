...
def subroutine_scat(x,theta,lam):  
    ...
    def G(u, k): # the the (3 × 3) Green’s function for Maxwell on R^3 for a given frequency k
        ...
        g = np.exp(1j * k * abs(u))  # Should have /abs(u) as well!!!
...
def FOM(x, color): 
    if color == 'blue':
        ...
        lam_naught = 0.465 #wavelength that target Gaussian is center
        ...
        for i in range(len(lam_step)):
            for j in range(len(theta_step)):
                if P_naught < subroutine_scat(x,theta_step[j],lam_step[i]):
                    P_naught = subroutine_scat(x,theta_step[j],lam_step[i]) #want P_naught to be maximum value of scattered power
        P_naught = ((np.pi) ** (1/2)) * P_naught
        figure_of_merit = 0.0 #initializing Riemann Sum
        for i in range(len(lam_step)):
            for j in range(len(theta_step)):
                P_scat = subroutine_scat(x,((theta_step[j-1]+theta_step[j])/2),((lam_step[i-1]+lam_step[i])/2)) / P_naught
                P_target = Gaussian((lam_step[i-1]+lam_step[i])/2,6161.31,lam_naught)
                integrand = P_scat - P_target
                figure_of_merit = figure_of_merit + ((np.abs(integrand)) ** 2) * delta_lam * delta_theta
    ...
    if color == 'green':
        ...
        delta_theta = 0.1 #step-size for theta in Riemann Sum
        delta_lam = 0.1 #step-size for lambda in Riemann Sum
        lam_naught = 0.5325 #wavelength that target Gaussian is center
        theta_step = np.arange(theta_i,theta_f,delta_theta) #creating array of theta values for Riemann Sum
        lam_step = np.arange(lam_i,lam_f,delta_lam) #creating array of lambda values for Riemann Sum
        P_naught = 0.0 #Initializing P_naught, a normalization factor
        for i in range(len(lam_step)):
        ...
        for i in range(len(lam_step)):
            for j in range(len(theta_step)):
                P_scat = subroutine_scat(x,((theta_step[j-1]+theta_step[j])/2),((lam_step[i-1]+lam_step[i])/2)) / P_naught
                P_target = Gaussian((lam_step[i-1]+lam_step[i])/2,1312.47,lam_naught)
                integrand = P_scat - P_target
                figure_of_merit = figure_of_merit + ((np.abs(integrand)) ** 2) * delta_lam * delta_theta
        return figure_of_merit
    if color == 'red':
        ...
        delta_theta = 0.1 #step-size for theta in Riemann Sum
        delta_lam = 0.1 #step-size for lambda in Riemann Sum
        lam_naught = 0.6625 #wavelength that target Gaussian is center
        theta_step = np.arange(theta_i,theta_f,delta_theta) #creating array of theta values for Riemann Sum
        lam_step = np.arange(lam_i,lam_f,delta_lam) #creating array of lambda values for Riemann Sum
        P_naught = 0.0 #Initializing P_naught, a normalization factor
        for i in range(len(lam_step)):
        ...
        for i in range(len(lam_step)):
            for j in range(len(theta_step)):
                P_scat = subroutine_scat(x,((theta_step[j-1]+theta_step[j])/2),((lam_step[i-1]+lam_step[i])/2)) / P_naught
                P_target = Gaussian((lam_step[i-1]+lam_step[i])/2,985.809,lam_naught)
                integrand = P_scat - P_target
                figure_of_merit = figure_of_merit + ((np.abs(integrand)) ** 2) * delta_lam * delta_theta
        return figure_of_merit
...
def MCMC(fTAR, delta, N, T, color):
    ...
    for i in range(T):
        ...
        if (np.abs(fTAR(x_1, color) / fTAR(x_0, color)) < 1): # definitely accept
            X = x_1 
            x_0 = x_1
            xxx.append(X)
            ...
            alpha = np.exp(-1 * 3500000. * (np.abs(fTAR(x_1, color)) - np.abs(fTAR(x_0, color))))
            # print(u)
            # print(alpha)
    ...
    i = 0
    for i in range(t):
        func_value = float(fTAR(xxx[i], color))
        y[i] = (func_value)
    #print(y)
